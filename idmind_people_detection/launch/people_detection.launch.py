...
def generate_launch_description():
    ...
    pp_pkg_share = get_package_share_directory("idmind_people_detection")

    ld.add_action(DeclareLaunchArgument('depth_enabled',              default_value="false",          description="Enable or disable the depth camera"))
    ...
    ld.add_action(DeclareLaunchArgument('point_cloud',                default_value="false",          description="Generate a point cloud from depth data. Requires depth_enabled"))
    ld.add_action(DeclareLaunchArgument('rgb_point_cloud',            default_value="false",          description="Colorize the point cloud using the RBG camera. Requires color_enabled and depth_enabled"))
    ...
    azure_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(pp_pkg_share, 'launch', 'start_azure.launch.py')]),
        launch_arguments={
    ...
    ppl_detection_node = Node(
        package='idmind_people_detection',
        executable='people_detection',
        parameters = [{

        }],
        # remappings = [("idmind_teleop/set_velocities", "motors_node/cmd_vel")]        
    )
    ld.add_action(ppl_detection_node)
    ...
    videoserver_node = Node(
        package = "web_video_server",
        executable = "web_video_server",
    )
    ld.add_action(videoserver_node)
