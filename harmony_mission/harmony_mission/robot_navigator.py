...
class BasicNavigator(Node):
    def __init__(self,node_name='basic_navigator'):
        ...
        self.nav_through_poses_client = ActionClient(self,
                                                     ...
                                                     '/harmony/navigate_through_poses')
        self.nav_to_pose_client = ActionClient(self, NavigateToPose, '/harmony/navigate_to_pose')
        self.follow_waypoints_client = ActionClient(self, FollowWaypoints, '/harmony/follow_waypoints')
        self.compute_path_to_pose_client = ActionClient(self, ComputePathToPose, '/harmony/compute_path_to_pose')
        self.compute_path_through_poses_client = ActionClient(self, ComputePathThroughPoses,
                                                              '/harmony/compute_path_through_poses')
        ...
        self.initial_pose_pub = self.create_publisher(PoseWithCovarianceStamped,
                                                      '/harmony/initialpose',
                                                      10)
        self.change_maps_srv = self.create_client(LoadMap, '/harmony/map_server/load_map')
        self.clear_costmap_global_srv = self.create_client(
            ClearEntireCostmap, '/harmony/global_costmap/clear_entirely_global_costmap')
        self.clear_costmap_local_srv = self.create_client(
            ClearEntireCostmap, '/harmony/local_costmap/clear_entirely_local_costmap')
        self.get_costmap_global_srv = self.create_client(GetCostmap, '/harmony/global_costmap/get_costmap')
        self.get_costmap_local_srv = self.create_client(GetCostmap, '/harmony/local_costmap/get_costmap')
    ...
    def waitUntilNav2Active(self):
        self._waitForNodeToActivate('/harmony/amcl')
        # self._waitForInitialPose()
        self._waitForNodeToActivate('/harmony/bt_navigator')
    ...
    def clearLocalCostmap(self):
        while not self.clear_costmap_local_srv.wait_for_service(timeout_sec=2.0):
    ...
    def clearGlobalCostmap(self):
        while not self.clear_costmap_global_srv.wait_for_service(timeout_sec=2.0):
    ...
    def _waitForNodeToActivate(self, node_name):
        ...
        self.info(node_service + ' service available, checking state...')
